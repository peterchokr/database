...
1. 사용자가 INSERT 명령 실행
2. BEFORE 트리거 실행
3. NEW.salary < 0이면 오류 발생 (데이터 삽입 안 됨)
4. 조건을 만족하면 데이터 삽입
...
('김철수', 5000000, '개발팀', '개발자', '2020-01-15'),
('이영희', 4000000, '인사팀', '인사담당자', '2020-06-20'),
('박민준', 4500000, '마케팅팀', '마케터', '2021-03-10');
```

### salary_history 테이블 (급여 변경 이력 기록)
...
신규 직원이 추가될 때 감사 로그에 자동 기록하는 트리거를 생성하세요.
...
VALUES ('정수진', 3500000, '재무팀', '회계담당자', '2024-01-06');

-- audit_log 테이블 확인
SELECT * FROM audit_log ORDER BY log_id DESC LIMIT 1;
-- 결과: 새로운 직원 추가 기록이 자동으로 저장됨
```

---

### 12-2. INSERT BEFORE 트리거 - 데이터 검증

신규 직원 추가 시 데이터를 검증하고 조건을 만족하지 않으면 거부하는 트리거를 생성하세요.
...
BEGIN
  ...
  -- 2. 급여 상한: 일반 직원은 1억원 이상 불가
  IF NEW.salary > 100000000 AND NEW.position NOT IN ('임원', 'CEO') THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Error: 1억원 이상의 급여는 임원급 이상만 가능합니다.';
  END IF;
...
VALUES ('김철수', -5000000, '개발팀', '개발자');
-- Error: 급여는 음수가 될 수 없습니다.

-- ❌ 실패: 부서 미지정
INSERT INTO employees (name, salary, position)
VALUES ('이영희', 4000000, '인사담당자');
-- Error: 부서 정보는 필수입니다.

-- ✅ 성공: 모든 조건 만족
INSERT INTO employees (name, salary, department, position)
VALUES ('박민준', 4500000, '마케팅팀', '마케터');
-- hire_date가 자동으로 오늘 날짜로 설정됨
```

---

### 12-3. INSERT AFTER 트리거 - 감사 로그

신규 직원 추가 후 모든 정보를 감사 로그에 기록하는 트리거를 생성하세요.
...
BEGIN
  ...
  ('employees', 'INSERT', 'hire_date', NULL, NEW.hire_date, NOW());
END;
```

---

### 12-4. UPDATE 트리거 - 변경 이력 추적

급여 변경 시 변경 이력을 자동으로 기록하는 트리거를 생성하세요.
...
-- salary_history에 자동으로 기록
-- emp_id: 1
-- emp_name: 김철수
-- old_salary: 5000000
-- new_salary: 5500000
-- changed_at: 2024-01-06 15:30:00
```

---

### 12-5. UPDATE BEFORE 트리거 - 수정 전 검증

급여 인상 시 과도한 인상을 방지하는 트리거를 생성하세요.
...
### 12-6. UPDATE AFTER 트리거 - 다중 기록

다양한 정보 변경을 별도로 기록하는 트리거를 생성하세요.
...
### 12-7. DELETE 트리거 - 아카이브

직원 삭제 전 모든 정보를 보관하는 트리거를 생성하세요.
...
생성한 트리거들을 확인하는 방법을 학습하세요.

```sql
-- 1. 현재 데이터베이스의 모든 트리거 확인
SHOW TRIGGERS;
