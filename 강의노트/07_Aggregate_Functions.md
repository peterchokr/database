...
NULL이 아닌 product_id의 개수를 조회하세요.
...
### 7-16. GROUP BY와 ORDER BY

상품별 판매량을 많은 순서대로 조회하세요.
...
### 7-17. HAVING으로 그룹 필터링

판매 건수가 3건 이상인 상품을 조회하세요.
...
### 7-18. HAVING과 HAVING 조건

판매액 합계가 500000 이상인 상품을 조회하세요.
...
### 7-19. WHERE와 HAVING 조합

2024년 이후 판매 데이터에서 판매액 합계가 600000 이상인 상품을 조회하세요.
...
### 7-20. GROUP BY 후 LIMIT

판매액이 가장 높은 상위 3개 상품을 조회하세요.
...
카테고리별 상품 개수, 평균 가격, 최대 가격을 조회하세요.
...
카테고리, 상품별로 판매량, 판매액, 평균 가격을 조회하세요.
...
### 7-27. GROUP_CONCAT으로 데이터 집약

상품별 판매 직원들을 쉼표로 구분하여 표시하세요.
...
```sql
SELECT 
    CASE WHEN sale_date < '2024-01-16' THEN '기간1' ELSE '기간2' END AS period,
...
**5번 과제**: Part 3의 실습 7-1부터 7-40까지 제공된 모든 쿼리를 직접 실행하고, 각 쿼리의 결과를 스크린샷으로 첨부하세요. 추가로 5개 이상의 창의적인 집계 쿼리를 작성하여 그 결과를 제시하고, 각 쿼리가 어떤 비즈니스 의사결정에 도움이 될 수 있는지 설명하세요.
