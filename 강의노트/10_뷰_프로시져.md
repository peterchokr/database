...
1. **복잡한 쿼리 단순화** - 여러 테이블을 JOIN하거나 복잡한 계산이 포함된 쿼리를 뷰로 만들어두면, 나중에는 SELECT * FROM view_name 한 줄로 끝납니다.
2. **보안** - 사용자에게 전체 테이블을 보여주지 않고, 필요한 컬럼(예: 급여 정보를 제외한 직원 정보)만 골라서 뷰로 만들어 제공할 수 있습니다.
3. **일관성** - 특정 조건(예: 퇴사자 제외)에 맞는 데이터만 조회하도록 강제할 수 있습니다.
...
1. **코드 재사용** - 같은 로직을 여러 애플리케이션에서 사용할 수 있습니다.
2. **보안** - 사용자에게 테이블 직접 접근 권한을 주지 않고, 특정 프로시저를 실행할 권한만 부여하여 데이터를 안전하게 보호할 수 있습니다.
3. **성능** - 한 번 컴파일되면 실행 계획이 캐시되므로 반복 실행 시 속도가 빠릅니다.
4. **복잡한 비즈니스 로직** - 조건문, 반복문으로 복잡한 작업을 수행할 수 있습니다.
