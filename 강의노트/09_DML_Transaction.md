...
이 섹션에서는 데이터베이스의 데이터를 안전하게 조작하는 DML 명령어(INSERT, UPDATE, DELETE)와 여러 작업을 하나의 논리적 단위로 처리하는 트랜잭션을 학습합니다.   
데이터를 정확하고 안전하게 입력, 수정, 삭제하는 방법과 오류 발생 시 이전 상태로 복구하는 방법을 다룹니다.
