...
1. **중복되는 기본 INNER JOIN 문제 제거**
   - 원래 5-1, 5-2: 유사한 강좌+교수 조회 → 통합
   - 원래 5-6: 교수별 담당 강좌 (중복) → 제거

2. **복잡한 CASE 문 제거**
   - 원래 5-12: CASE 문을 이용한 평가 등급 → 제거 (복잡도 높음)
   - 원래 5-20: CASE 문을 이용한 평가 → 제거

3. **거의 유사한 LEFT JOIN 문제 통합**
   - 원래 5-23: 수강생 없는 강좌 → 제거 (5-14와 유사)
   - 원래 5-25: CONCAT을 이용한 수강현황 → 제거 (복잡함)

4. **복잡한 자기조인 제거**
   - 원래 5-30: 같은 전공 학생들의 수강 강좌 (자기조인) → 제거 (초급자 맞지 않음)

5. **중복되는 집계 문제 제거**
   - 원래 5-35: 최신 강좌를 수강하는 학생 → 제거
   - 원래 5-39: 전공별 강좌 편성 (복합 GROUP BY) → 제거

**결과:**
- INNER JOIN: 15개 → 11개 (4개 감소)
- LEFT JOIN: 10개 → 9개 (1개 감소)
- 복합 JOIN: 15개 → 10개 (5개 감소)
- **총 40개 → 30개 (10개 감소)**
